package cleaner.implementations
...
/**
  * Created by aurore on 09/06/16.
  *
  * CharacterCleaner is taken from CMU Ark
  ...
  *
  */
trait CharacterCleaner extends Cleaner {
  ...
  this: DataContainer =>
    ...
    override def clean(newColumn: Option[String] = None,
                       struct: DataSelect = DataSelect()): DataContainer with Cleaner = {
...
object TwitterRegex {
  ...
  val hash= "[\\@\\#]"
  val searchPattern = OR(url, nonAscii, emoticon._1, emoticon._2, emoticon._3, emoticon._4, hash).r
